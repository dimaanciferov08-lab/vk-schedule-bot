...
# === КЛАСС ДЛЯ СОЗДАНИЯ РЕФЕРАТОВ С WIKIPEDIA ===
class ReferatGenerator:
    ...
    def clean_content(self, text):
        ...
        return '. '.join(sentences[:20]) + '.'  # Берем первые 20 предложений
    
    def create_referat_structure(self, topic, wiki_data=None):
        """Создание структуры реферата"""
        
        if wiki_data and wiki_data.get('exists'):
            # Используем данные из Wikipedia
            content = self.clean_content(wiki_data['content'])
            sentences = content.split('. ')
            
            introduction = f"""Актуальность темы '{topic}' обусловлена ее значимостью в современном мире. {sentences[0] if sentences else 'Данная тема представляет научный и практический интерес.'}
...
Предмет исследования: ключевые аспекты и особенности {topic.lower()}"""
            
            # Формируем главы из контента Wikipedia
            chapters = []
            if len(sentences) > 5:
                chapters = [
                    {
                        'title': 'Основные понятия и теоретические аспекты',
                        'content': f"{'. '.join(sentences[1:4])}.\n\nРассматриваются базовые понятия, определения и теоретическая база изучения данной темы."
                    },
                    {
                        'title': 'Анализ и современное состояние',
                        'content': f"{'. '.join(sentences[4:8])}.\n\nАнализируется современное состояние, тенденции развития и практическое значение."
                    }
                ]
            else:
                chapters = self._generate_template_chapters(topic, content)
                
            conclusion = f"""В ходе выполнения реферата на тему "{topic}" были достигнуты поставленные цели и решены задачи исследования.

Основные выводы:
1. {sentences[0] if sentences else 'Тема имеет важное научное и практическое значение.'}
2. Обнаружены перспективные направления для дальнейшего изучения
3. Практическая значимость работы подтверждена анализом материала"""
            
        else:
            # Шаблонная структура
            introduction = self._generate_template_introduction(topic)
            chapters = self._generate_template_chapters(topic)
            conclusion = self._generate_template_conclusion(topic)
        ...
        structure = {
            ...
            'sources': self._generate_sources(wiki_data),
            'appendix': self._generate_appendix(topic)
        }
        return structure
    
    def _generate_template_introduction(self, topic):
        """Шаблонное введение"""
...
Предмет исследования - основные закономерности и особенности {topic.lower()}"""
    ...
    def _generate_template_chapters(self, topic, content=None):
        """Шаблонные главы"""
        return [
            {
                'title': 'Теоретические аспекты изучения темы',
                'content': f"""Тема '{topic}' представляет значительный интерес для современной науки. В данной главе рассматриваются основные понятия, исторический контекст и теоретические основы изучения данной проблематики.

1.1. Основные понятия и определения
{topic} - это комплексное явление, требующее многоаспектного изучения. 

1.2. Исторический контекст
История изучения {topic.lower()} насчитывает несколько десятилетий и продолжает развиваться."""
            },
            {
                'title': 'Анализ современных подходов и методов',
                'content': f"""В данной главе анализируются современные подходы к изучению {topic.lower()}. Рассматриваются различные методологии, практические аспекты и результаты исследований.

2.1. Современное состояние проблемы
На современном этапе {topic.lower()} активно развивается и находит применение в различных сферах.

2.2. Практическое применение
Практическая значимость {topic.lower()} проявляется в возможности решения актуальных задач."""
            }
        ]
    
    def _generate_template_conclusion(self, topic):
...
- Рассмотреть возможность практического применения полученных знаний"""
    ...
    def _generate_sources(self, wiki_data=None):
        ...
        if wiki_data and wiki_data.get('exists'):
            sources.insert(0, f"Wikipedia - {wiki_data['title']}")
...
Для углубленного изучения темы рекомендуется ознакомиться с дополнительной литературой."""
    ...
    def generate_referat(self, topic):
        ...
        try:
            ...
            referat_structure = self.create_referat_structure(topic, wiki_data)
            ...
            if wiki_data and wiki_data.get('exists'):
                source_info = f"📚 Источник: Wikipedia ({wiki_data['title']})"
            else:
                source_info = "📚 Использована шаблонная структура"
...
• Уточните требования у преподавателя"""
...
📝 Особенности:
• Объем: 8-12 страниц
• Форматирование: Times New Roman, 14pt
